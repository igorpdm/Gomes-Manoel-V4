...
async function execute(interaction) {
  ...
  const generateQueueEmbed = (page) => {
    ...
    const currentTime = getCurrentTime(currentMusic); //tempo atual da música
    const progressBar = createProgressBar(
      currentTime,
      currentMusic.musica.duracao
    );

    const embed = new EmbedBuilder()
      .setDescription(
        `🎶 **Tocando agora:** \n**[${currentMusic.musica.nome}](${
          currentMusic.musica.url
        })\n\`${new Date(currentTime * 1000)
          .toISOString()
          .substr(14, 5)} / ${new Date(currentMusic.musica.duracao * 1000)
          .toISOString()
          .substr(14, 5)}\` ${progressBar}**\n\n` +
          "**🎶 Músicas na fila:**\n" +
          currentQueue
            .map(
              (music, index) =>
                `**${start + index + 1}.** **\`${formatDuration(
                  music.duracao
                )}\`** [**${music.nome}**](${music.url})`
            )
            .join("\n")
      )
      .setFooter({
        text: `Página ${page + 1} de ${Math.ceil(queue.length / pageSize)} (${
          queue.length
        } músicas) | Tempo estimado: ${formattedDuration}`,
      })
