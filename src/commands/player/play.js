  ...
  try {
    ...
    if (!state.isPlaying) {
      ...
      const addedToQueueEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `\`${formatDuration(musica.duracao)}\` [${musica.nome}](${
            musica.url
          })`
        )
        .setAuthor({
        ...
        .setColor(0x57f287)
        .setFooter({
          text: `Posição na fila: ${queue.length}`,
  ...
  try {
    ...
    if (state.isPlaying) {
      const playlistEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `${videos.items.length} músicas da [playlist](${url}) foram adicionados à fila!`
        )
        .setAuthor({
          name: interaction.user.username,
          iconURL: interaction.user.avatarURL(),
...
async function playNext(interaction, connection, player, command) {
  ...
  const musicPath = path.join(
    __dirname,
    `../../musicas/${currentMusic.musica.audioFile}`
  );
  ...
  const tocando = new EmbedBuilder()
    .setTitle("🎵 Tocando")
    .setDescription(
      `\`${formatDuration(currentMusic.musica.duracao)}\` [${
